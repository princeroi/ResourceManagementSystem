...
namespace App\Filament\Resources\SmeRestocks\Tables;
            ...
            ->recordActions([
                ...
                Action::make('view')
                    ...
                    ->modalContent(function ($record) {
                        ...
                        foreach ($record->smeRestockItem as $i => $item) {
                ...
                Action::make('return_item')
                    ...
                    ->form(function ($record) {
                        ...
                        foreach ($record->smeRestockItem as $item) {
                            ...
                            $itemOptions[$item->id] =
                                "{$item->smeItem?->sme_item_name} ({$item->smeItemVariant?->sme_item_size}) — delivered: {$delivered}";
                        }
                    ...
                    ->action(function ($record, array $data, Action $action) {
                        ...
                        foreach ($returns as $idx => $row) {
                            ...
                            $restockItem   = $record->smeRestockItem->firstWhere('id', $restockItemId);
                            ...
                            if ($returnQty > (int) $restockItem->delivered_quantity) {
                                Notification::make()
                                    ...
                                    ->body("'{$restockItem->smeItem?->sme_item_name}' only has {$restockItem->delivered_quantity} delivered.")
                                    ->danger()
                                    ->send();
                                $action->halt();
                                return;
                            ...
                            if ($variant && (int) $variant->sme_item_quantity < $returnQty) {
                                Notification::make()
                                    ...
                                    ->body("'{$restockItem->smeItem?->sme_item_name}' only has {$variant->sme_item_quantity} in stock.")
                                    ->danger()
                                    ->send();
                                $action->halt();
                                return;
                        ...
                        foreach ($returns as $row) {
                            ...
                            $noteRows[] = [
                                'label'        => "{$restockItem->smeItem?->sme_item_name} ({$restockItem->smeItemVariant?->sme_item_size})",
                                'qty'          => $returnQty,
                                'reason'       => $reason,
                                'remarks'      => $remarks,
                                'stock_before' => $stockBefore,
                ...
                Action::make('view_logs')
                    ...
                    ->modalContent(function ($record) {
                        $logs = SmeRestockLogs::where('sme_restock_id', $record->id)
                            ->with('user')
                            ->latest()
                            ->get();
