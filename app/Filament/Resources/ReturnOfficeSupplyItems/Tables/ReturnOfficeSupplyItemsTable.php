...
namespace App\Filament\Resources\ReturnOfficeSupplyItems\Tables;
...
class ReturnOfficeSupplyItemsTable
{
        ...
        return $table
            ...
            ->recordActions([
                ...
                Action::make('view')
                    ...
                    ->icon('heroicon-o-eye')
                    ->modalWidth('3xl')
                    ->modalHeading('Office Supply Return')
                    ->modalContent(function ($record) {
                        ...
                        foreach ($record->returnOfficeSupplyItemLine as $i => $line) {
                            $itemName   = e($line->officeSupplyItem?->office_supply_name ?? '—');
                            $variant    = e($line->officeSupplyItemVariant?->office_supply_variant ?? '—');
                            ...
                            $rows .= "
                                <tr style='background:{$bg};'>
                                    ...
                                    <td style='padding:9px 12px;border-bottom:1px solid #e5e7eb;font-size:12px;text-align:center;color:#374151;'>{$variant}</td>
                                ...
                                <div style='background:linear-gradient(135deg,#1e3a5f 0%,#1e40af 100%);
                                    ...
                                    <div style='display:flex;justify-content:space-between;align-items:flex-start;'>
                                        <div>
                                            ...
                                            <div style='font-size:12px;color:#93c5fd;margin-top:4px;'>Return Record #{{$record->id}}</div>
                                        </div>
                                    ...
                                    <table style='width:100%;border-collapse:collapse;'>
                                        <thead>
                                            <tr style='background:#1e3a5f;'>
                                                ...
                                                <th style='padding:7px 12px;text-align:center;font-size:10px;font-weight:700;color:#fff;text-transform:uppercase;letter-spacing:.05em;width:80px;'>Variant</th>
                ...
                Action::make('accept')
                    ...
                    ->form(function ($record) {
                        ...
                        foreach ($record->returnOfficeSupplyItemLine as $line) {
                            ...
                            $itemName   = $line->officeSupplyItem?->office_supply_name ?? '—';
                            $variant    = $line->officeSupplyItemVariant?->office_supply_variant ?? '—';
                            ...
                            $fields[] = TextInput::make("line_{$line->id}_accept")
                                ->label("{$itemName} — {$variant}{$employee} (remaining: {$remaining})")
                                ->numeric()
                                ->default($remaining)
                                ->minValue(0)
                                ->maxValue($remaining)
                            ...
                            if ($acceptQty > 0) {
                                ...
                                if ($addToStock) {
                                    ...
                                    if ($variant) {
                                        ...
                                        $variant->increment('office_supply_quantity', $acceptQty);
                ...
                Action::make('view_logs')
                    ...
                    ->modalContent(function ($record) {
                        $logs = ReturnOfficeSupplyItemLog::where('return_office_supply_item_id', $record->id)
                            ->with('user')
                            ->latest()
                            ->get();
