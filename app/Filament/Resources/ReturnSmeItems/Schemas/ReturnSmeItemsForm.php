...
namespace App\Filament\Resources\ReturnSmeItems\Schemas;
            ...
            ->components([
                ...
                Select::make('status')
                    ...
                    ->afterStateUpdated(function ($state, callable $set, callable $get) {
                        ...
                        $lines = $get('returnSmeItemLine') ?? [];
                        foreach ($lines as $key => $line) {
                            $qty = (int) ($line['quantity'] ?? 0);
                            if ($state === 'returned') {
                                $set("returnSmeItemLine.{$key}.returned_quantity",  $qty);
                                $set("returnSmeItemLine.{$key}.remaining_quantity", 0);
                            } else {
                                $set("returnSmeItemLine.{$key}.returned_quantity",  0);
                                $set("returnSmeItemLine.{$key}.remaining_quantity", $qty);
                            }
                        }
                    }),
                ...
                FileUpload::make('po_file_path')
                    ->label('PO File')
                    ->nullable()
                    ->directory('sme-po-files')
                    ->acceptedFileTypes(['application/pdf', 'image/*']),

                FileUpload::make('dr_file_path')
                    ->label('DR File')
                    ->nullable()
                    ->directory('sme-dr-files')
                    ->acceptedFileTypes(['application/pdf', 'image/*']),
                ...
                // ── Line Items ────────────────────────────────────────────
                Repeater::make('returnSmeItemLine')
                    ->label('Items to Return')
                    ->relationship('returnSmeItemLine')
