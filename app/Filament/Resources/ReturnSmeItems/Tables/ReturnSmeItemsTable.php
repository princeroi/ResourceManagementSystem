...
namespace App\Filament\Resources\ReturnSmeItems\Tables;
        ...
        return $table
            ...
            ->recordActions([
                ...
                Action::make('view')
                    ...
                    ->modalContent(function ($record) {
                        ...
                        $poDate = $record->po_date
                            ? \Carbon\Carbon::parse($record->po_date)->format('F d, Y')
                            : '—';
                        ...
                        foreach ($record->returnSmeItemLine as $i => $line) {
                            $itemName   = e($line->smeItem?->sme_item_name ?? '—');
                            $size       = e($line->smeItemVariant?->sme_item_size ?? '—');
                                ...
                                <div style='background:linear-gradient(135deg,#1e3a5f 0%,#1e40af 100%);
                                    ...
                                    <div style='display:flex;justify-content:space-between;align-items:flex-start;'>
                                        <div>
                                            ...
                                            <div style='font-size:12px;color:#93c5fd;margin-top:4px;'>SME Return</div>
                                        </div>
                                    ...
                                    <div style='display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:14px;'>
                                        ...
                                        <div style='background:rgba(255,255,255,.08);border-radius:8px;padding:10px 14px;'>
                                            <div style='font-size:10px;color:#93c5fd;font-weight:600;text-transform:uppercase;letter-spacing:.06em;margin-bottom:3px;'>PO Number</div>
                                            <div style='font-size:13px;font-weight:600;color:#fff;'>{$poNumber}</div>
                                        </div>
                                        <div style='background:rgba(255,255,255,.08);border-radius:8px;padding:10px 14px;'>
                                            ...
                                            <div style='font-size:13px;font-weight:600;color:#fff;'>{$drNumber}</div>
                                        </div>
                                        <div style='background:rgba(255,255,255,.06);border-radius:8px;padding:10px 14px;'>
                                        ...
                                        <div style='background:rgba(255,255,255,.06);border-radius:8px;padding:10px 14px;'>
                                            ...
                                            <div style='font-size:12px;font-weight:500;color:#fff;'>{$note}</div>
                                        </div>
                                    </div>
                                </div>
                ...
                Action::make('accept')
                    ...
                    ->form(function ($record) {
                        ...
                        foreach ($record->returnSmeItemLine as $line) {
                            ...
                            $itemName   = $line->smeItem?->sme_item_name ?? '—';
                            $size       = $line->smeItemVariant?->sme_item_size ?? '—';
                            ...
                            if ($acceptQty > 0) {
                                ...
                                if ($addToStock) {
                                    ...
                                    if ($variant) {
                                        ...
                                        $variant->increment('sme_item_quantity', $acceptQty);
                ...
                Action::make('view_logs')
                    ...
                    ->modalContent(function ($record) {
                        $logs = ReturnSmeItemLog::where('return_sme_item_id', $record->id)
                            ->with('user')
                            ->latest()
                            ->get();
